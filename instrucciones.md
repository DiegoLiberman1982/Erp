...
`transaction_type` argument to choose the right template set for the document context
(`'sales'` for sales documents and `'purchase'` for purchase documents):
```python
...
#### Import UI
- Mantener las pantallas de importación limpias (similar a SalesPriceListManager): botones, carga de archivo y tabla; sin textos largos.
- Cualquier explicación o tutorial va en secciones de ayuda/tooltip, no en el layout principal del importador.
