...
const CustomerGroupModal = ({
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={editingGroup ? 'Editar Grupo de Clientes' : 'Crear Grupo de Clientes'}
      size="lg"
    >
      <div className="space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
              <option value={0}>Grupo Individual (contiene clientes)</option>
        ...
        {groupFormData.is_group === 0 && (
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
              {customerGroups?.filter(group => group.is_group === 1 && group.name !== groupFormData.name).map((group) => (
        ...
        {groupFormData.is_group === 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-black text-gray-700 mb-1">Lista de Precios por Defecto</label>
              <select
                value={groupFormData.default_price_list || ''}
                onChange={(e) => onFormChange({ ...groupFormData, default_price_list: e.target.value })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                <option value="">Seleccionar lista de precios...</option>
                {salesPriceLists?.map((priceList) => (
                  <option key={priceList.name} value={priceList.price_list_name}>
