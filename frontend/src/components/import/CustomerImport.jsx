...
import { Download, FileSpreadsheet, Save, Trash2, Users } from 'lucide-react'
...
const REQUIRED_KEYS = ['customer_name', 'tax_id']
...
export default function CustomerImport() {
  ...
  const generateRowId = useCallback(
    () => `customer-row-${Date.now()}-${Math.random().toString(16).slice(2)}`,
    []
  )

  const createEmptyRow = useCallback(
    () => ({ id: generateRowId(), customer_name: '', tax_id: '', custom_condicion_iva: '' }),
    [generateRowId]
  )
  ...
  const columns = useMemo(
    () => [
      { key: 'customer_name', label: 'Nombre', type: 'text', width: 260 },
  ...
  const parseXlsx = async (file) => {
    ...
    for (let i = headerRowIndex + 1; i < grid.length; i++) {
      const row = grid[i] || []
      const obj = { id: generateRowId(), customer_name: '', tax_id: '', custom_condicion_iva: '' }
      headers.forEach((key, colIdx) => {
        if (!key) return
        obj[key] = row[colIdx] ?? ''
      })
      obj.customer_name = String(obj.customer_name || '').trim()
      obj.tax_id = normalizeTaxId(obj.tax_id)
    ...
    async (event) => {
      ...
      try {
        ...
        if (parsed.length === 0) {
          ...
          showSuccess(`Archivo cargado (${parsed.length} clientes)`)
        }
      } catch (e) {
        console.error('CustomerImport parse error', e)
  ...
  const handleDownloadTemplate = () => {
    try {
      ...
      XLSX.writeFile(workbook, 'import_clientes.xlsx')
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchWithAuth(API_ROUTES.importCustomers, {
        method: 'POST',
      ...
      if (!res.ok || !data?.success) {
        showError(data?.message || 'Error importando clientes')
        return
      }
      ...
      showSuccess(`Clientes: ${summary.created || 0} creados, ${summary.exists || 0} existentes, ${summary.errors || 0} errores`)
    } catch (e) {
      console.error('import customers error', e)
      showError('Error importando clientes')
    } finally {
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xl font-bold text-gray-800">Clientes</div>
              </div>
            </div>
