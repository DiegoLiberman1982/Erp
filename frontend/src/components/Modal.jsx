...
const Modal = ({
  ...
  const [position, setPosition] = useState({ x: 200, y: 16 })
  ...
  const clampPositionToViewport = useCallback(() => {
    ...
    const clampedX = Math.max(padding, Math.min(position.x, safeMaxX))
    const clampedY = Math.max(padding, Math.min(position.y, safeMaxY))
  ...
  const handleMouseMove = useCallback((e) => {
    ...
    const safeMaxX = Math.max(padding, maxX)
    const safeMaxY = Math.max(padding, maxY)
    const clampedX = Math.max(padding, Math.min(newX, safeMaxX))
    const clampedY = Math.max(padding, Math.min(newY, safeMaxY))
    setPosition({ x: clampedX, y: clampedY })
  }, [isDragging])
  ...
  }, [isOpen, clampPositionToViewport, isMinimized, size])

  if (typeof document === 'undefined' || !isOpen) return null

  const modalClasses = {
    small: 'w-96 h-auto max-h-[calc(100vh-24px)]',
    md: 'w-11/12 max-w-3xl h-auto max-h-[calc(100vh-24px)]',
    lg: 'w-11/12 max-w-4xl h-auto max-h-[calc(100vh-24px)]',
    default: 'w-11/12 max-w-7xl h-auto max-h-[calc(100vh-24px)]',
    large: 'w-11/12 max-w-6xl h-auto max-h-[calc(100vh-24px)]',
    xl: 'w-11/12 max-w-9xl h-auto max-h-[calc(100vh-24px)]'
  }
  ...
  const modalMarkup = (
    <>
      ...
      >
        ...
        {!isMinimized && <div className="p-4 overflow-auto flex-1 min-h-0">{children}</div>}
      </div>
    </>
  )
