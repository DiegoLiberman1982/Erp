...
import { ShoppingCart, FileText, FileSpreadsheet } from 'lucide-react'
import usePriceListReports from './usePriceListReports'
import { AuthContext } from '../../../AuthProvider'

export default function PurchasePriceList() {
  const [search, setSearch] = useState('')
  const { fetchWithAuth } = useContext(AuthContext)

  // Use the purchase price lists endpoint
  const endpoint = API_ROUTES.purchasePriceLists || '/api/inventory/purchase-price-lists/all'
  ...
  const exportCsvForList = (priceListName) => {
    const listItems = filteredItems.filter(item => item.price_list_name === priceListName)
    exportToCsv(listItems, `lista_precios_compra_${priceListName}.csv`)
  }
  ...
  const exportXlsxForList = async (priceListName) => {
    try {
      ...
      const response = await fetchWithAuth(`/api/reports/price-lists/export-purchase-xlsx?${params.toString()}`)
      ...
      let filename = `Precios_Compra_${priceListName}.xlsx`
      ...
      {loading && (
        <div className="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl overflow-hidden border border-gray-200/30 p-8">
          <div className="flex items-center justify-center">
            ...
            <div className="text-xl font-bold text-gray-900">Cargando lista de precios de compra...</div>
          </div>
        </div>
      )}
      ...
      {!loading && !error && (
        <div className="space-y-6">
          <div className="bg-white/80 rounded-2xl shadow-lg border border-gray-200/40 p-4">
            <h4 className="text-lg font-bold text-gray-900 mb-4">Lista de Precios de Compra</h4>
