...
// Use the missing purchase prices endpoint
const endpoint = '/api/reports/price-lists/missing-purchase-prices'

export default function MissingPurchasePriceReport() {
  ...
  const exportVisible = () => {
    ...
    exportToCsv(rows, 'missing_purchase_prices.csv')
  }

  const exportVisibleXlsx = () => {
    const rows = displayItems.map(i => ({ item_code: i.item_code, item_name: i.item_name }))
    exportToXlsx(rows, 'missing_purchase_prices.xlsx')
  }
        ...
        <div className="space-y-6">
          <div className="bg-white/70 backdrop-blur-xl shadow-2xl rounded-3xl border border-gray-200/30 p-6 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-2xl font-black text-gray-900">Items sin precio de compra</h3>
                <p className="text-sm text-gray-600">Filtra, revisa y exporta los items que faltan precio de compra.</p>
              </div>
            </div>
