...
import { formatDate, formatCurrency, formatNumber, mapVoucherTypeToSigla, formatVoucherNo } from './inventoryUtils'

/**
 * Tabla de movimientos de inventario con paginacion y consumo completo de la card contenedora.
 */
export default function MovementsTable({ movements, selectedItem, movementWarehouseTab, warehouses, openRemitoForEdit, openStockReconciliation, hideZeroMovements = false }) {
  ...
  // Resetear a la primera pagina cuando cambia el item, el filtro de deposito o los datos
  useEffect(() => {
    setCurrentPage(1)
  }, [selectedItem, movementWarehouseTab, movements])

  // Filtrar por deposito/tab seleccionado (siempre se ejecuta antes de cualquier return para no romper el orden de hooks)
  const filteredMovements = useMemo(() => {
    ...
    // 2. Hide Stock Reconciliation with qty=0 if hideZeroMovements is enabled
    const baseMovements = Array.isArray(movements)
      ? movements.filter(m => {
          // Las reservas siempre se incluyen (no tienen docstatus tradicional)
          if (m.is_reservation) return true
          // Si hideZeroMovements está activo, filtrar movimientos con qty exactamente 0
          // SOLO para Stock Reconciliation - otros tipos (FCV, ING, etc) siempre se muestran
          if (hideZeroMovements) {
  ...
  if (!selectedItem || selectedItem === 'new') {
    return (
      <div className="text-center py-12 text-gray-500">
        ...
        <p>Selecciona un item del panel izquierdo para ver sus movimientos</p>
      </div>
    )
  }
  ...
  const handleRowClick = (movement) => {
    try {
      ...
      const isStockReconciliation = vtype === 'stock reconciliation' || vtype.includes('reconciliation')
      const isRemitoType = vtype.includes('remito') || vtype.includes('purchase receipt') || vtype.includes('receipt') || vtype.includes('delivery')
      if (voucherNo && isStockReconciliation && typeof openStockReconciliation === 'function') {
        openStockReconciliation(voucherNo)
        return
      }
